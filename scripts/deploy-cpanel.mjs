#!/usr/bin/env node
// SlySplit SPA deploy — adapted from CaseMaker's deploy-cpanel.mjs.
//
...
// CPANEL_TOKEN, WEB_ROOT (e.g. /home/USER/public_html/slysplit).
//
// Usage:
...
if (SITE_ROOT_TAILS.has(tail)) {
  console.error(
    ...
    `       Use a subdirectory like ${env.WEB_ROOT}/slysplit. Update .env and rerun.`,
  );
  process.exit(2);
}
...
// Derive the Vite base from WEB_ROOT: /home/U/public_html/slysplit → /slysplit/
...
if (!SKIP_BUILD) {
  ...
  const result = spawnSync('npm', ['run', 'build', '-w', '@slysplit/web'], {
    cwd: REPO_ROOT,
    stdio: 'inherit',
...
// .htaccess: asset caching + SPA fallback (client routes exist in SlySplit).
